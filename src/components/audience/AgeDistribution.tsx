...
export default function AgeDistribution({ uploadId }: { uploadId: number }) {
  const [ageData, setAgeData] = useState<{ ageRange: string; count: number }[]>([]);

  useEffect(() => {
    const fetchData = async () => {
      if (!uploadId) return;
      try {
        const data = await audienceService.getAgeDistribution(uploadId);
        setAgeData(data);
      } catch (err) {
